...
var visa32 = ffi.Library('visa32', {
  ...
  'viReadSTB': [ViStatus, [ViSession, ref.types.uint64]],
...
var nisa32c = require(binding_path); //"./build/nisa32/v1.0.0/Release/node-v14-win32-x64/nisa32");
...
function VisaPortFactory() {
  ...
  VisaPort.prototype.query = function (queryString, callback) {
    ...
    var viError = visa32.viQueryf(this.viSession.deref(), queryString + "", "%s", replyBuff);
    if (viError) {
    	return callback(viError);
    }  else {
      var replyString = '';
  ...
  VisaPort.prototype.clear = function(callback) {
   ...
   VisaPort.prototype.readStatusByte = function(callback) {
      assert.equal(typeof (callback), 'function');
      var statusByte = ref.alloc(ViUInt16);
      var viError = visa32.viReadSTB(this.viSession.deref(), statusByte.address());
      if (viError) {
      	return callback(viError);
      }  
      else {
        //if ((status >= VI_SUCCESS) && (statusByte & 0x40))
        callback(null, statusByte.deref());  
      }
   }
  ...
  */
  ...
  util.inherits(factory.VisaPort, nisa32c.Nisa32c);
  util.inherits(factory.VisaPort, EventEmitter);
}
