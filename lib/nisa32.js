...
var visa32 = ffi.Library('visa32', {
  ...
  'viFindRsrc': ['int',['int', 'string', 'string', 'int', ByteArray]], //  (ViSession sesn, ViString expr, ViPFindList vi, ViPUInt32 retCnt, ViChar _VI_FAR desc[]);
...
function VisaPortFactory() {
  ...
  function list(callback) {
    ...
    if (viError) {
    	return callback(viError); // err = new Error('Not implemented yet');
    }

    var VI_FIND_BUFLEN = 256; // from visa.h
    var descBuff = new ByteArray(VI_FIND_BUFLEN); 
    var bla, blabla = "";
    var count = 0;
console.log(resourceManager);
    viError = visa32.viFindRsrc('256', "?*", bla, count, descBuff);
    if (viError) {
    	return callback(viError);
    }
    console.log("found instruments: " + count);
    console.log(bla);
    console.log(blabla);
